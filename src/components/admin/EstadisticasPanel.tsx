...
const EstadisticasPanel = () => {
  ...
  const addMutation = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from("estadisticas").insert({ categoria, label, value, suffix: suffix || null });
      if (error) throw error;
    },
    onSuccess: () => {
      ...
      setShowForm(false);
      toast({ title: "Estadística agregada" });
    },
  });
